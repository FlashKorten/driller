...
whereExcludeList = [ " AND NOT EXISTS (SELECT id_game FROM nn_map_author    WHERE id_game = g.id AND id_author    = (-1 * ?))"
                   , " AND NOT EXISTS (SELECT id_game FROM nn_map_publisher WHERE id_game = g.id AND id_publisher = (-1 * ?))"
